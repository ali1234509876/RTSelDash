import { $ as commonjsGlobal, P as getDefaultExportFromCjs, r as reactExports, T as React, U as jsxRuntimeExports } from "./worker-entry-LGGiUUMr.js";
import { R as Route, u as useI18n, d as useNavigate, a as useAuth, c as currentPeriod, l as lastNPeriods, p as periodToDate, m as monthLabel, t as toast, b as formatDate, f as formatCurrency } from "./router-C21oMGn1.js";
import { P as ProtectedShell, a as useQuery, B as Building2, u as updateProfile, s as setMonthlyTarget, h as getMonthlyTargetsHistory } from "./supabase-data-BRA3oWFj.js";
...
import { a as useProfile } from "./use-profiles-fTwdOUvA.js";
import { a as monthRangeOf, f as filterByPeriod, c as computeMetrics, e as efficiencyRatio, p as pacingRatio, b as monthlyTrend } from "./metrics-BdRbdHws.js";
...
import { c as createLucideIcon, f as clsx, B as Button } from "./button-C5Miz1Tm.js";
...
function tenureLabel(hiredAt, yearsLabel, monthsLabel) {
  if (!hiredAt) return "—";
  ...
  const m = months % 12;
  if (y2 === 0) return `${m}${monthsLabel}`;
  if (m === 0) return `${y2}${yearsLabel}`;
  return `${y2}${yearsLabel} ${m}${monthsLabel}`;
}
...
function Inner() {
  ...
  } = useProfile(id, period);
  const empTxs = reactExports.useMemo(() => txs.filter((tx) => tx.sales_rep_id === id), [txs, id]);
  const range3 = reactExports.useMemo(() => monthRangeOf(period), [period]);
  const empMonthTxs = reactExports.useMemo(() => filterByPeriod(empTxs, range3), [empTxs, range3]);
  const metrics = reactExports.useMemo(() => computeMetrics(empMonthTxs), [empMonthTxs]);
  ...
  const trendPeriods = reactExports.useMemo(() => lastNPeriods(6, periodToDate(period)), [period]);
  const trendKey = reactExports.useMemo(() => trendPeriods.join(","), [trendPeriods]);
  const {
    data: targetsHistory = {}
  } = useQuery({
  ...
  const trend = reactExports.useMemo(() => monthlyTrend(empTxs, trendPeriods, targetsHistory), [empTxs, trendPeriods, targetsHistory]);
  const trendData = reactExports.useMemo(() => trend.map((p) => ({
    period: p.period,
    label: monthLabel(p.period, lang),
    achievement: p.achievement,
    target: p.target
  })), [trend, lang]);
  const [targetDraft, setTargetDraft] = reactExports.useState("");
  ...
  const updateDepartment = async (newId) => {
  ...
  reactExports.useEffect(() => {
    if (!loading && profile && primaryRole === "dept_head" && managedDepartmentIds.length > 0 && (!profile.department_id || !managedDepartmentIds.includes(profile.department_id))) {
  ...
  if (loading) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "px-10 py-8 text-sm text-muted-foreground", children: t("common.loading") });
  }
  if (!profile) {
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "px-10 py-8", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mt-10 text-center text-muted-foreground", children: t("emp.detail.notFound") })
    ] });
  }
  const pState = pacingState(pacing);
  const pacingLabel = pState === "ahead" ? t("kpi.pacing.ahead") : pState === "behind" ? t("kpi.pacing.behind") : t("kpi.pacing.onTrack");
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "px-10 py-8 max-w-[1600px] mx-auto", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { type: "button", onClick: () => navigate({
      to: "/team"
    }), className: "inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground mb-6", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(ArrowLeft, { className: "size-4" }),
      t("emp.detail.back")
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { className: "mb-8 flex items-end justify-between gap-6 flex-wrap", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-5", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "size-16 rounded-full bg-accent border border-border-strong flex items-center justify-center text-base font-semibold text-foreground", children: (profile.full_name ?? "·").slice(0, 2).toUpperCase() }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-[10px] uppercase tracking-widest text-muted-foreground flex items-center gap-2", children: [
            t("emp.detail.title"),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: `inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium ${profile.is_active ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30" : "bg-muted/40 text-muted-foreground border border-border"}`, children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(Power, { className: "size-2.5" }),
              profile.is_active ? t("emp.detail.active") : t("emp.detail.inactive")
            ] })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { className: "text-2xl font-light text-foreground mt-0.5", children: profile.full_name ?? "—" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-muted-foreground mt-1", children: dept ? departmentLabel(dept, lang) : t("dept.none") })
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-end gap-4 flex-wrap", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "min-w-[160px]", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-[10px] uppercase tracking-widest text-muted-foreground mb-1.5", children: t("emp.detail.period") }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Input, { type: "month", value: periodMonth, onChange: (e) => {
            const v = e.target.value;
            if (v) setPeriod(`${v}-01`);
          }, className: "h-9" })
        ] }),
        isCeo && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "min-w-[200px]", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-[10px] uppercase tracking-widest text-muted-foreground mb-1.5", children: t("dept.assign") }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("select", { value: profile.department_id ?? "__none__", onChange: (e) => updateDepartment(e.target.value), className: "w-full h-9 rounded-md border border-input bg-background px-3 text-sm", children: [
        ...
        isCeo && /* @__PURE__ */ jsxRuntimeExports.jsxs(Button, { size: "sm", variant: profile.is_active ? "outline" : "default", onClick: toggleActive, className: "h-9", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Power, { className: "size-3.5 me-2" }),
          profile.is_active ? t("emp.detail.deactivate") : t("emp.detail.activate")
        ] })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "glass-card rounded-2xl p-5 mb-8 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetaItem, { icon: /* @__PURE__ */ jsxRuntimeExports.jsx(Phone, { className: "size-3.5" }), label: t("emp.detail.phone"), value: profile.phone ?? t("emp.detail.notSet"), dim: !profile.phone }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetaItem, { icon: /* @__PURE__ */ jsxRuntimeExports.jsx(Calendar, { className: "size-3.5" }), label: t("emp.detail.hired"), value: profile.hired_at ? formatDate(profile.hired_at, lang) : t("emp.detail.notSet"), dim: !profile.hired_at }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetaItem, { icon: /* @__PURE__ */ jsxRuntimeExports.jsx(Clock, { className: "size-3.5" }), label: t("emp.detail.tenure"), value: tenureLabel(profile.hired_at, t("emp.detail.tenureYears"), t("emp.detail.tenureMonths")), dim: !profile.hired_at }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetaItem, { icon: /* @__PURE__ */ jsxRuntimeExports.jsx(Building2, { className: "size-3.5" }), label: t("dept.label"), value: dept ? departmentLabel(dept, lang) : t("dept.none"), dim: !dept })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-2 lg:grid-cols-4 gap-6 mb-8", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(KpiCard, { label: t("kpi.totalAchievement"), value: `${formatCurrency(metrics.totalAchievement, lang)} ${t("common.currency")}`, accent: "success" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(KpiCard, { label: t("kpi.monthlyTarget"), labelSecondary: monthLabel(period, lang), value: target > 0 ? `${formatCurrency(target, lang)} ${t("common.currency")}` : t("emp.detail.noTarget"), accent: "primary", hint: /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: variance >= 0 ? "text-success" : "text-destructive", children: [
        variance >= 0 ? "+" : "",
        formatCurrency(variance, lang),
        " ",
        t("common.currency")
      ] }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(KpiCard, { label: t("kpi.efficiencyRatio"), value: `${ratio.toFixed(1)}%`, accent: "primary", hint: target > 0 ? /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: pState === "ahead" ? "text-success" : pState === "behind" ? "text-destructive" : "text-muted-foreground", children: [
        pacingLabel,
        " · ",
        pacing.toFixed(2),
        "×"
      ] }) : null }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(KpiCard, { label: t("kpi.closeRate"), value: `${metrics.closeRate.toFixed(1)}%`, accent: metrics.cancelledFiles > 0 ? "warning" : "neutral", hint: metrics.cancelledFiles > 0 ? /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "text-warning", children: [
        metrics.cancelledFiles,
        " ",
        t("kpi.cancelledFiles"),
        " · ",
        formatCurrency(metrics.cancelledValue, lang),
        " ",
        t("common.currency")
      ] }) : null })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "glass-card rounded-2xl p-5 mb-8 flex items-end gap-3 flex-wrap", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 min-w-[220px]", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-[10px] uppercase tracking-widest text-muted-foreground mb-1.5", children: [
          t("emp.detail.editTarget"),
          " · ",
          monthLabel(period, lang)
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(Input, { type: "number", min: "0", step: "0.01", placeholder: String(target), value: targetDraft, onChange: (e) => setTargetDraft(e.target.value), className: "h-9" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(Button, { onClick: saveTarget, disabled: !targetDraft, children: t("tx.save") })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "glass-card p-8 rounded-3xl flex flex-col items-center", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(RadialProgress, { value: ratio, size: 220 }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mt-8 w-full grid grid-cols-3 gap-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(CountChip, { label: t("kpi.completedFiles"), value: metrics.completedFiles, accent: "success" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(CountChip, { label: t("kpi.pendingFiles"), value: metrics.pendingFiles, accent: "primary" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(CountChip, { label: t("kpi.cancelledFiles"), value: metrics.cancelledFiles, accent: metrics.cancelledFiles > 0 ? "warning" : "neutral" })
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "lg:col-span-2 glass-card rounded-3xl overflow-hidden", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "px-8 py-5 border-b border-border", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-[11px] uppercase tracking-widest text-foreground font-bold", children: t("emp.detail.recent") }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(TransactionsTable, { rows: empMonthTxs.slice(0, 12) })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "glass-card rounded-3xl overflow-hidden", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "px-8 py-5 border-b border-border flex items-baseline justify-between", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-[11px] uppercase tracking-widest text-foreground font-bold", children: t("emp.detail.trend") }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-muted-foreground", children: t("emp.detail.trendSubtitle") })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "p-6 h-[280px]", children: /* @__PURE__ */ jsxRuntimeExports.jsx(ResponsiveContainer, { width: "100%", height: "100%", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(ComposedChart, { data: trendData, margin: {
        top: 10,
        right: 16,
        left: 8,
        bottom: 0
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(YAxis, { stroke: "var(--muted-foreground)", fontSize: 11, tickLine: false, axisLine: false, tickFormatter: (v) => formatCurrency(v, lang), width: 70 }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(Tooltip, { contentStyle: {
          background: "var(--popover)",
          border: "1px solid var(--border)",
          borderRadius: "0.5rem",
          fontSize: "12px"
        }, formatter: (value, name) => [`${formatCurrency(value, lang)} ${t("common.currency")}`, name === "achievement" ? t("kpi.totalAchievement") : t("kpi.monthlyTarget")] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(Bar, { dataKey: "achievement", fill: "var(--primary)", radius: [4, 4, 0, 0] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(Line, { type: "monotone", dataKey: "target", stroke: "var(--muted-foreground)", strokeDasharray: "4 4", dot: {
          r: 3
        } })
      ] }) }) })
    ] })
...
function MetaItem({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start gap-3", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mt-0.5 text-muted-foreground", children: icon }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "min-w-0", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `text-sm mt-0.5 truncate ${dim ? "text-muted-foreground" : "text-foreground"}`, children: value })
    ] })
  ] });
}
function CountChip({
  ...
  const accentClass = accent === "success" ? "text-success" : accent === "warning" ? "text-warning" : accent === "primary" ? "text-primary" : "text-muted-foreground";
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-center", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-[10px] uppercase tracking-widest text-muted-foreground", children: label }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `text-lg font-light tabular mt-1 ${accentClass}`, children: value })
  ] });
}
export {
  EmployeeDetailPage as component
};
